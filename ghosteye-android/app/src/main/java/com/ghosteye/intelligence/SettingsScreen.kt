...
fun SettingsScreen(
    ...
    var mobileStatus by remember { mutableStateOf<JSONObject?>(null) }
    ...
    LazyColumn(
        ...
        item {
            SectionCard {
                ...
                Text(ServerConfig.OWNER_LABEL, style = MaterialTheme.typography.bodyLarge)
        ...
        diagnostics?.let { d ->
            item {
                SectionCard {
                    ...
                    mobileStatus?.let { status ->
                        val compatibility = status.optJSONObject("compatibility")
                        val supported = compatibility?.optBoolean("supported", true) != false
                        val upgrade = compatibility?.optBoolean("upgrade_recommended", false) == true
                        val serverVersion = status.optString("server_version", "?")
                        DiagnosticLine("תאימות אפליקציה", if (!supported) "דורש עדכון" else if (upgrade) "עדכון מומלץ" else "תואם לשרת $serverVersion", supported)
                        val degraded = status.optJSONArray("degraded_components")
                        val degradedCount = degraded?.length() ?: 0
                        DiagnosticLine("מצב מערכת", if (degradedCount == 0) "כל הרכיבים תקינים" else "$degradedCount רכיבים דורשים תשומת לב", degradedCount == 0)
                    }
                }
            }
        }
...
private fun friendlyError(e: Exception, fallback: String): String = when (e) {
    is ApiException -> buildString { append("שגיאת שרת ${e.code}: ${e.message}"); e.requestId?.let { append(" • מזהה: ${it.take(12)}") } }
