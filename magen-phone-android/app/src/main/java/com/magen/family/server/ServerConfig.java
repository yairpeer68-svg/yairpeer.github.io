...
public final class ServerConfig {
    ...
    public static String baseUrl(Context c){
        ...
        if(raw==null||raw.trim().isEmpty()) return BuildConfig.MAGEN_SERVER_URL;
        String u=normalizeUrl(raw);
        // v4.2.1 paired migration: an APK update preserves SharedPreferences. If the
        // previous paired VPS endpoint is still stored, move to the new coexistence
        // endpoint and force a fresh enrollment. Do not rewrite arbitrary custom URLs.
        if(isLegacyPairedUrl(u)){
            prefs.edit()
                    .putString(K_URL,BuildConfig.MAGEN_SERVER_URL)
                    .putBoolean(K_ENROLLED,false)
                    .putInt(K_POLICY_VER,0)
                    .apply();
            ServerVerdictCache.clear(c);
            ServerRuleCache.clear(c);
            return BuildConfig.MAGEN_SERVER_URL;
        }
        return u;
    }
    private static boolean isLegacyPairedUrl(String u){
        return "https://51.21.194.27".equalsIgnoreCase(u)
                || "https://51.21.194.27:443".equalsIgnoreCase(u);
    }
