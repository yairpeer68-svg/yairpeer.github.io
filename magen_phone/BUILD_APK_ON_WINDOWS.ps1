...
Info 'Windows APK builder v4.2.0-full-hardening'
...
if (-not (Test-Path $VisualModel)) {
    ...
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/GantMan/nsfw_model/releases/tags/1.1.0" -Headers @{ 'User-Agent'='MagenBuilder/4.2' }
...
$outApk = Join-Path $ProjectDir 'magen-v4.2.0-full-hardening-debug.apk'
Copy-Item $builtApk $outApk -Force
$hash = (Get-FileHash -Path $outApk -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  magen-v4.2.0-full-hardening-debug.apk" | Set-Content -Path (Join-Path $ProjectDir 'magen-v4.2.0-full-hardening-debug.apk.sha256') -Encoding ascii
