...
/**
 * ActivityReporter — דוח פעילות תקופתי לשרת Magen.
 *
 * הדוח כולל חסימות, ניסיונות עקיפה והתראות אבטחה, ונשלח רק דרך
 * ServerEventReporter/NotificationHelper בלבד; אין ערוץ התראות חיצוני על המכשיר.
 */
public class ActivityReporter extends JobService {
    ...
    public static void schedule(Context ctx) {
        try {
            ...
            if (js == null) return;
            ComponentName cn = new ComponentName(ctx, ActivityReporter.class);
            // התזמון אינו תלוי בזמינות רשת; אם ה-VPS אינו זמין האירוע ייכשל
            // באופן בטוח וההגנה המקומית ממשיכה לפעול.
            JobInfo job = new JobInfo.Builder(JOB_ID, cn)
                ...
                .build();
            js.schedule(job);
            Log.d(TAG, "Daily activity report scheduled");
        } catch (Exception e) {
    ...
    public static void sendReport(Context ctx) {
        ...
        NotificationHelper.notifyDigest(ctx, sb.toString());
        Log.d(TAG, "Report sent to server");
