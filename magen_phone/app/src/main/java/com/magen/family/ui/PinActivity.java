...
public class PinActivity extends BaseActivity {
    ...
    private boolean regularFailureArmed = false;
    ...
    private void addDigit(String digit) {
        ...
        // אחרי 4 ספרות שגויות מותר רק להמשיך ישירות לקוד החירום.
        // עוצרים את הטיימר בזמן ההקלדה, אבל לא מוחקים את מצב הכישלון.
        if (regularFailureArmed) cancelFailureTimerOnly();
        ...
        if (enteredPin.length() == PIN_LENGTH) {
            if (mode.equals("verify")) {
                ...
                // isLockedOut() מנקה את הקלט; במקרה כזה לא סופרים ניסיון נוסף.
                if (enteredPin.length() != PIN_LENGTH) return;
                // נכשל. אולי המשתמש ממשיך לקוד חירום בן 6 ספרות.
                schedulePendingFailure();
            } else {
                processPin();
            }
        } else if (enteredPin.length() == PIN_LENGTH + 1
                && mode.equals("verify") && regularFailureArmed) {
            // תן חלון קצר להקלדת הספרה השישית, בלי לאפשר להעלים את הכישלון.
            schedulePendingFailure();
        } else if (enteredPin.length() == EMERGENCY_LEN && mode.equals("verify")) {
            cancelFailureTimerOnly();
            tryVerifyEmergencyPin();
        }
    }
    ...
    private void schedulePendingFailure() {
        cancelFailureTimerOnly();
        regularFailureArmed = true;
        pendingFailure = () -> {
            pendingFailure = null;
            if (!regularFailureArmed) return;
            regularFailureArmed = false;
            countFailedAttempt();
        };
        failureHandler.postDelayed(pendingFailure, FAILURE_GRACE_MS);
    }

    private void cancelFailureTimerOnly() {
    ...
    private void disarmPendingFailure() {
        cancelFailureTimerOnly();
        regularFailureArmed = false;
    }

    private void commitPendingFailure() {
        if (!regularFailureArmed) return;
        cancelFailureTimerOnly();
        regularFailureArmed = false;
        countFailedAttempt();
    ...
    private void deleteDigit() {
        if (regularFailureArmed) {
            // אחרי שקוד 4 ספרות כבר נכשל, Backspace אינו יכול לבטל את הכישלון.
    ...
    private void tryVerifyEmergencyPin() {
        ...
        // PIN שגוי לחלוטין — ספור ניסיון פעם אחת
        disarmPendingFailure();
        countFailedAttempt();
    }

    private boolean isLockedOut() {
        android.content.SharedPreferences prefs = MagenApp.getInstance().getPrefs();
        long wallUntil = prefs.getLong("pin_lock_until", 0);
        long elapsedUntil = prefs.getLong("pin_lock_until_elapsed", 0);
        long elapsedSet = prefs.getLong("pin_lock_set_elapsed", 0);
        int lockBoot = prefs.getInt("pin_lock_boot_count", -1);
        int bootNow = currentBootCount();
        long nowElapsed = android.os.SystemClock.elapsedRealtime();

        boolean sameBoot = lockBoot >= 0 && bootNow == lockBoot
            && elapsedSet > 0 && nowElapsed >= elapsedSet;
        long remainingMs = sameBoot
            ? elapsedUntil - nowElapsed
            : wallUntil - System.currentTimeMillis();

        if (remainingMs > 0) {
            long remaining = remainingMs / 1000 / 60;
    ...
    private int currentBootCount() {
        try {
            return android.provider.Settings.Global.getInt(
                getContentResolver(), android.provider.Settings.Global.BOOT_COUNT);
        } catch (Exception e) {
            return -1;
        }
    }
    ...
    private void countFailedAttempt() {
        ...
        if (attempts >= MAX_ATTEMPTS) {
            ...
            long elapsedSet = android.os.SystemClock.elapsedRealtime();
            MagenApp.getInstance().getPrefs().edit()
                .putLong("pin_lock_until", lockUntil)
                .putLong("pin_lock_set_elapsed", elapsedSet)
                .putLong("pin_lock_until_elapsed", elapsedSet + lockoutMs)
                .putInt("pin_lock_boot_count", currentBootCount())
            ...
            // התראת אבטחה לשרת
            com.magen.family.service.NotificationHelper.notifyUrgent(
    ...
    private void resetAttempts() {
        MagenApp.getInstance().getPrefs().edit()
            ...
            .putLong("pin_lock_set_elapsed", 0)
            .putLong("pin_lock_until_elapsed", 0)
            .putInt("pin_lock_boot_count", -1)
            .apply();
    }
    ...
    protected void onPause() {
        super.onPause();
        // יציאה מהמסך אינה דרך לבטל ניסיון PIN שכבר נכשל.
        if (!authenticated && regularFailureArmed) commitPendingFailure();
    ...
    protected void onDestroy() {
        disarmPendingFailure();
        failureHandler.removeCallbacksAndMessages(null);
        super.onDestroy();
    }
}
