 ...
 *   • בדיקת החתימה מקבלת את SHA-256 של תעודת ה-release בזמן build.
 *     build מסוג release נכשל אם fingerprint לא הוגדר, וב-runtime כשל קריאה
 *     של החתימה נחשב חשוד במקום לעבור בשקט.
 */
public class IntegrityGuard {

    private static final String TAG = "IntegrityGuard";

    // מוזרק מה-build באמצעות -PreleaseCertSha256. release ללא fingerprint נכשל בבנייה.
    private static final String EXPECTED_SIG_SHA256 = BuildConfig.EXPECTED_SIG_SHA256;
     ...
     * שנכשלה יצרה התראה דחופה בכל מחזור Watchdog,
    ...
    public static boolean isSignatureTampered(Context ctx) {
        if (EXPECTED_SIG_SHA256 == null || EXPECTED_SIG_SHA256.trim().isEmpty()) {
            // ב-debug מותר לעבוד בלי תעודת release. ב-release זה fail-closed
            // (ובנוסף ה-build עצמו אמור להיכשל לפני יצירת APK).
            return !BuildConfig.DEBUG;
        }
        try {
            String actual = getOwnSignatureSha256(ctx);
            if (actual == null) return !BuildConfig.DEBUG;
