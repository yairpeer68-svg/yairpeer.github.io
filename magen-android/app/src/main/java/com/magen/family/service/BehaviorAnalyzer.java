 ...
 *   • Escalation: מאגד אירועים, שולח להורה רק ברגעים חשובים — בלי SMS spam.
...
public class BehaviorAnalyzer {
    ...
    private static final long MIN_ALERT_INTERVAL_MS   = 30 * 60 * 1000L;  // אזהרה להורה מקסימום אחת ל-30 דק
    ...
    /**
     * אגירת התראות — לא שולחים SMS על כל בליפ.
    ...
    private void scheduleParentAlert(String detail) {
        ...
        if (now - lastAlert >= MIN_ALERT_INTERVAL_MS) {
            // שלח עכשיו
            NotificationHelper.notifyPartnerDigest(MagenApp.getInstance(), digest);
            prefs.edit()
    ...
    public static void flushPendingDigest(Context ctx) {
        ...
        NotificationHelper.notifyPartnerDigest(ctx, digest);
        p.edit()
            .putString(K_PENDING_DIGEST, "")
    ...
    /**
     * עזר — שליחת התראה לשותף אחריות.
     */
    public static void notifyPartner(Context ctx, String message) {
        NotificationHelper.notifyPartnerUrgent(ctx, message);
    }
}
