...
import com.magen.family.service.TelegramNotifier;
...
public class CovenantCenterActivity extends BaseActivity {
    ...
    private void showPanic() {
        ...
        // התראה לשותף שנעזר בכפתור — חלק מהשקיפות
        try {
            NotificationHelper.notifyPartnerDigest(this,
        ...
        new android.app.AlertDialog.Builder(this)
            ...
            .setNeutralButton(R.string.cov_panic_call, (d, w) -> callPartner())
            .show();
    }

    private void callPartner() {
        String phone = MagenApp.getInstance().getPrefs()
            .getString(MagenApp.KEY_PARENT_PHONE, "");
        if (phone.isEmpty()) {
            Toast.makeText(this, R.string.phone_missing, Toast.LENGTH_LONG).show();
            return;
        }
        try {
            startActivity(new Intent(Intent.ACTION_DIAL, Uri.parse("tel:" + phone)));
        } catch (Exception e) {
            Toast.makeText(this, R.string.phone_missing, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private Button moodButton(int labelRes, String moodTag) {
        ...
        b.setOnClickListener(v -> {
            ...
            if ("מתקשה".equals(moodTag)) {
                TelegramNotifier.send(this, "😕 צ׳ק-אין: מתקשה היום. אולי כדאי ליצור קשר.");
            }
            build();
        });
        return b;
